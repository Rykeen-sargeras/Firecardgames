...
io.on("connection", (socket) => {
  ...
  socket.on("submit", (data) => {
    ...
    if (data.card === BLANK_CARD && data.custom) {
    ...
    if (data.card !== BLANK_CARD) room.whiteDiscard.push(data.card);
  ...
  socket.on("pick", (odumid) => {
    ...
    winner.score++;
    io.to(socket.roomCode).emit("round-winner", { name: winner.name, score: winner.score, odumid: odumid });
    
    if (winner.score >= WIN_POINTS) {
      io.to(socket.roomCode).emit("game-winner", { name: winner.name });
      setTimeout(() => {
        room.started = false;
        Object.values(room.players).forEach(p => { p.ready = false; });
        broadcastLobby(socket.roomCode);
  ...
  socket.on("admin", (data) => {
    ...
    if (data.action === "reset") {
      ...
      room.countdownValue = 0;
      room.pendingPlayers = {};
      Object.values(room.players).forEach(p => { p.ready = false; p.hand = []; p.score = 0; });
